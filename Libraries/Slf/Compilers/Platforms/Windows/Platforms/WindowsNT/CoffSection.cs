...
namespace AllenCopeland.Abstraction.Slf.Platforms.WindowsNT
{
        ...
        IDisposable
    {
        ...
        private static string GetNameFor(byte[] name)
        {
            ...
            for (index = 0; index < result.Length; index++)
                if (name[index] != 0)
                    result[index] = (char)name[index];
                else
                {
                    broke = true;
                    break;
                }
            string resultName = new string(result, 0, broke ? index : result.Length);
            return resultName;
        }
        /// <summary>
        ...
        public virtual bool HasCustomRead { get { return false; } }
        public virtual void CustomRead(EndianAwareBinaryReader reader) { }
    }
}
