...
namespace AllenCopeland.Abstraction.Slf.Ast.Members
{
            ...
            TGrandParent
    {
        ...
        /// initialized.
        /// </summary>
        protected virtual bool AreParametersInitialized
        {
            get 
            { 
                lock (SyncObject)
        ...
        public bool LastIsParams
        {
            get
            {
                ...
                var lipService = assembly.GetLastIsParamsService();
                if (lipService == null)
                    throw new NotSupportedException("There is no service defined which supports querying the LastIsParams concept.");
                return lipService.SignatureSupportsParameterArray(this, assembly);
            }
            set
            {
                ...
                if (this.parameters == null || this.Parameters.Count == 0)
                    throw new InvalidOperationException("Cannot set last params on a signature with zero parameters.");

                var assembly = this.Assembly;
                var lipService = assembly.GetLastIsParamsService();
                if (lipService == null)
                    throw new NotSupportedException("There is no service defined which supports modifying the LastIsParams concept.");
                lipService.ChangeSignatureParameterArraySupport(this, this.Assembly, value);
            }
        }
