...
namespace AllenCopeland.Abstraction.Slf.Ast
{
        ...
        IMetadatumDefinition
    {
        ...
        IEnumerable<MetadatumTypedParameter> IMetadatum.Parameters
        {
            get { 
                foreach (var parameter in this.parameters)
                    if (!(parameter is IMetadatumDefinitionNamedParameter))
                    {
                        yield return new MetadatumTypedParameter { ParameterType = parameter.ParameterType, Value = parameter.Value };
                    }
            }
        }

        IEnumerable<MetadatumNamedParameter> IMetadatum.NamedParameters
        {
            get {
                foreach (var parameter in this.parameters)
                    if (parameter is IMetadatumDefinitionNamedParameter)
                    {
                        ...
                        yield return new MetadatumNamedParameter { ParameterType = nParameter.ParameterType, MemberName = nParameter.Name, Value = nParameter.Value };
                    }
            }
        }
