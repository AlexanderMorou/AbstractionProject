...
namespace AllenCopeland.Abstraction.Slf._Internal.Cli
{
        ...
        _ICliManager
    {
        ...
        public CliManager(ICliRuntimeEnvironmentInfo runtimeEnvironment)
        {
            ...
            this.RegisterService<IIdentityMetadataService>(IdentityManagerServiceGuids.MetadatumService, new MetadataService(this));
            this.RegisterService<ITypeNameBuilderService>(IdentityManagerServiceGuids.TypeNameBuilderService, new TypeNameService(this));
        }
                ...
                foreach (var assembly in loadedAssemblies)
                    if (assembly.MetadataRoot == root)
                        ...
                        foreach (var module in assembly.Modules.Values)
                        {
                            ...
                            if (cliModule == null)
                                continue;
                            if (cliModule.Metadata.MetadataRoot == assembly.MetadataRoot)
                                continue;
        ...
        {
            return CliGateway.ParseTypeIdentifier(typeIdentity, this);
        }
        ...
        public IType ObtainTypeReference(ICliMetadataTypeRefTableRow typeIdentity)
        {
            switch (typeIdentity.ResolutionScope)
            {
                case CliMetadataResolutionScopeTag.Module:
                    {
                        /* *
                         * Shouldn't appear in compliant compilers which compress the metadata into the smallest form.
                         * *
                         * This should support compilers that are not CLS compliant.
                         * */
        ...
        public IAssembly ObtainAssemblyReference(string filename)
        {
            ...
            if (!this.fileIdentifiers.TryGetValue(filename, out uniqueIdentifier))
            {
                if (File.Exists(filename))
                {
                    ...
                    {
                        /* *
                         * x64 is backwards compatible with x86/Any CPU, therefore
        ...
        public RuntimeCoreType ObtainCoreType(IType type)
        {
            ...
            if (metadata == null)
                /* Metadata on intermediate types isn't presently implemented */
                return RuntimeCoreType.None;
        ...
        public IIdentityMetadataService MetadatumHandler
        {
            get
            {
                return this.GetService<IIdentityMetadataService>(IdentityManagerServiceGuids.MetadatumService);
            }
        }
        ...
        public IEnumerable<IType> GetCoreTypeInterfaces(RuntimeCoreType coreType, IAssembly relativeAssembly = null)
        {
            ...
            switch (coreType)
            {
                ...
                case RuntimeCoreType.Decimal:
                    if ((runtimeInfo.Version & (CliFrameworkVersion.v4_0_30319 | CliFrameworkVersion.v4_5 | CliFrameworkVersion.v4_6)) != (CliFrameworkVersion)0)
                        if (runtimeInfo.UseCoreLibrary)
