            ...
            IIdentityMetadataService
        {
            ...
            public MetadatumInfo GetMetadatumInfo(IType metadatumType)
            {
                ...
                if (metadatumType.IsDefined(attributeUsageMetadatumType))
                {
                    ...
                    if (!validOn.IsEmpty)
                        targets = (AttributeTargets)(int)validOn.Value;
                    var inheritableParameter = (from n in attrUsageMetadatum.NamedParameters
                                                where n.MemberName == "Inherited"
                                                select n).FirstOrDefault();
                    var allowMultiple = (from n in attrUsageMetadatum.NamedParameters
                                         where n.MemberName == "AllowMultiple"
                                         select n).FirstOrDefault();
                    return new MetadatumInfo(isMetadata, allowMultiple.IsEmpty ? false : (bool)allowMultiple.Value, inheritableParameter.IsEmpty ? true : (bool)inheritableParameter.Value, TransposeTargets(targets));
                }
                else
            ...
            {
                get { return IdentityManagerServiceGuids.MetadatumService; }
            }
        }
    }
}
