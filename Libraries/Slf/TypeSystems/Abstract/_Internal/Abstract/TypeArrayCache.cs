...
namespace AllenCopeland.Abstraction.Slf._Internal.Abstract
{
    ...
    {
        /* *
         * Instance cache dictionaries, to prevent multiple instances of the same type.
         * */
        private IDictionary<int, IArrayType> normalArrayCache;
        /* *
         * The lengths and lowers cache which outlines the arrays which have a set of lower-bounds defined along with the lengths.
         * Contrary to the name the lower-bounds are the first key, the lengths are the second key.
         * */
        ...
        public void Dispose()
        {
            ...
            if (this.lengthsAndLowersCache != null)
            {
                ...
                for (int i = 0; i < elements.Length; i++)
                {
                    ...
                    currentElement.Value.Dispose();
                    this.lengthsAndLowersCache.Remove(currentElement.Keys.Key1, currentElement.Keys.Key2);
                }
            }
            //Release delegates.
            creatorVector = null;
