...
namespace AllenCopeland.Abstraction.Slf.Abstract
{
    ...
    public static partial class TypeExtensions
    {
        ...
        private static string GetGenericOpenGlyph(TypeParameterDisplayMode displayMode)
        {
            switch (displayMode)
            {
        ...
        public static bool ContainsGenericParameters(this IType type)
        {
            if (type.IsGenericConstruct && type is IGenericType)
            {
        ...
        public static string GetNamespaceFinalPathPart(this string path, string separator = ".")
        {
            if (!path.Contains(separator))
                return path;
            return path.Substring(path.IndicesOf(separator).Last().Previous);
        }
