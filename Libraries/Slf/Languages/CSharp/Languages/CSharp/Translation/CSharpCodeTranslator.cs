...
namespace AllenCopeland.Abstraction.Slf.Languages.CSharp.Translation
{
        ...
        IntermediateCodeTranslatorBase
    {
        ...
        public override void Translate(IDecoratingExpression expression)
        {
            ...
            {
                var leftSide = ObtainDecorations(expression, DecorationDisplaySide.Before);
                var rightSide = ObtainDecorations(expression, DecorationDisplaySide.After);
        ...
        public override void Translate(IMethodPointerReferenceExpression expression)
        {
            if (expression.Reference != null)
            {
                ...
                {
                    expression.Reference.Source.Accept(this);
                    this.WriteOperator(".");
                }
        ...
        public override void Translate(IMethodInvokeExpression expression)
        {
            expression.Reference.Accept(this);
            this.WriteOperator("(");
            this.Translate(expression.Arguments);
            this.WriteOperator(")");
        }
        ...
        public override void Translate(ICreateInstanceExpression expression)
        {
            ...
            if (expression == null || expression.MemberAssignments == null || expression.MemberAssignments.Count > 0)
            {
                this.WriteOperator("{");
                bool first = true;
        ...
        public override void Translate(ILinqExpression expression)
        {
            ...
            this.DenoteNewLine();
            VisitLinqExpressionBody(body);
            this.DecreaseIndent();
        }

        private void VisitLinqExpressionBody(ILinqBody body)
        {
            bool first = true;
        ...
        public override void Translate(ILinqFusionSelectBody expression)
        {
            ...
            {
                this.DenoteNewLine();
                this.VisitLinqExpressionBody(expression.Next);
            }
            this.DenoteNewLine();
        }
        ...
        private void TranslateFrom(ILinqRangeVariable rangeVariable, IExpression rangeSource)
        {
            ...
            this.Write(" ");
            rangeVariable.Accept(this);
            this.WriteKeyword(" in ");
            rangeSource.Accept(this);
        }
        ...
        private void TranslateJoin(ILinqRangeVariable rangeVariable, IExpression rangeSource, IExpression leftSelector, IExpression rightSelector)
        {
            ...
            leftSelector.Accept(this);
            this.WriteKeyword(" equals ");
            rightSelector.Accept(this);
            this.DenoteNewLine();
        }
        ...
        public override void Translate(IPrimitiveExpression<IType> expression)
        {
            this.WriteKeyword(CSharpKeywords.Typeof);
            this.WriteOperator("(");
            this.Translate(expression.Value);
            this.WriteOperator(")");
        }
        ...
        {
            bool includedBrace = false;
            if (statement is IConditionBlockStatement && statement.Count == 1 && statement[0] is IBlockStatement && ((IConditionBlockStatement)(statement)).HasNext)
                canOmitBraces = false;
            if (!(canOmitBraces && statement.Count == 1 && (statement.Locals.Count == 0 || !statement.Locals.Values.Any(k => k.AutoDeclare))) && !mustOmitBraces)
            {
                includedBrace = true;
                this.WriteOperator("{");
                this.DenoteNewLine();
            ...
            foreach (var local in from local in statement.Locals.Values
                                  ...
                                  select local)
                local.GetDeclarationStatement().Accept(this);
        ...
        public override void Translate(ISimpleIterationBlockStatement statement)
        {
            ...
            if (firstDecl == null)
                this.Translate(new CommentExpression("No declaration defined.", DecorationDisplaySide.After));
            else if (statement.Target.DeclaredLocals.Count > 1)
                this.Translate(new CommentExpression("ISimpleIterationBlockStatement had more than one declaration, remainder ignored!", DecorationDisplaySide.After));
        ...
        {
            return breakableBlockStatement.Accept(breakScanner, this.Options);
        }
            ...
            if (statement.ClauseCount > 0)
            {
                foreach (var typedClause in ((IControlledDictionary<IType, ITypedCatchExceptionBlockStatement>)statement))
                {
                    ...
                    if (clause is ITypeNamedCatchExceptionBlockStatement)
                        TranslateLocalDeclarationInternal(((ITypeNamedCatchExceptionBlockStatement)clause).LocalVariableDeclaration, false);
                    else
                        this.Translate(type);
                    this.WriteOperator(")");
                    this.DenoteNewLine();
        ...
        private void TranslateLocalDeclarationInternal(ILocalDeclarationsStatement statement, bool endLine = true)
        {
            ...
            else if (!endLine)
            {
                ...
                this.Translate(new CommentExpression("Multiple typing methods or type encountered in local declarations statement.", DecorationDisplaySide.After));
            }
            else
        ...
        private void TranslateLocalDeclarationInternal(ILocalMember member, bool emitType, bool endLine)
        {
            ...
            {
                this.WriteOperator(" = ");
                member.InitializationExpression.Accept(this);
            }
            if (endLine)
            {
                this.WriteOperator(";");
        ...
        public override void Translate(IIntermediateAssembly assembly)
        {
            ...
            if (Options.AllowPartials || assembly.IsRoot)
            {
                ...
                try
                {
                    ...
                    if (this.Options.EmitGenerationTime)
                    {
        ...
        public override void Translate(IIntermediateClassType @class)
        {
            if (Options.AllowPartials || @class.IsRoot)
            {
                ...
                if ((tc > 0 || mc > 0) && Options.AllowPartials || @class.IsRoot)
                {
                    ...
                    if (Options.AllowPartials && (!@class.IsRoot || @class.Parts.Count > 0 || @class.ForcedPartial))
                    {
                    ...
                    if ((baseRequired || classAsIClass.GetDirectlyImplementedInterfaces().Count > 0) && (Options.AllowPartials && @class.IsRoot || !Options.AllowPartials))
                    {
                        this.WriteOperator(" :");
                        this.DenoteNewLine();
                        IncreaseIndent();
                    ...
                    if (@class.IsGenericConstruct)
                    {
                        ...
                        foreach (var parameter in @class.TypeParameters.Values)
                        {
                            bool currentConstraints = this.TranslateConstraints(parameter, lastParam == parameter);
                            if (currentConstraints && !anyConstraints)
                                anyConstraints = true;
                        }
                        this.DecreaseIndent();
                        if (anyConstraints)
                    ...
                    else if (alpha && specific)
                    {
                        ...
                        foreach (var declaration in from declaration in
                            ...
                            if (declaration is IIntermediateMember)
                                ((IIntermediateMember)declaration).Accept(this);
                            else if (declaration is IIntermediateType)
                                ((IIntermediateType)(declaration)).Accept(this);
                    }
                    else if (alpha)
                    {
                        foreach (var declaration in from declaration in
        ...
        private IEnumerable<IIntermediateDeclaration> GetAllInterfaceDeclarations(IIntermediateInterfaceType @interface)
        {
            return (from m in
                        Options.AllowPartials ? @interface.Members.ExclusivelyOnParent(@interface) : from q in @interface.Members
                                                                                             select new KeyValuePair<IGeneralMemberUniqueIdentifier, MasterDictionaryEntry<IIntermediateMember>>(q.Key, new MasterDictionaryEntry<IIntermediateMember>(q.Value.Subordinate, (IIntermediateMember)q.Value.Entry))
                    select m.Value.Entry).Concat<IIntermediateDeclaration>(
                                                                        from t in
                                                                            Options.AllowPartials ? @interface.Types.ExclusivelyOnParent() : from q in @interface.Types
                                                                                                                                         select new KeyValuePair<IGeneralTypeUniqueIdentifier, MasterDictionaryEntry<IIntermediateType>>(q.Key, new MasterDictionaryEntry<IIntermediateType>(q.Value.Subordinate, (IIntermediateType)q.Value.Entry))
                                                                        select t.Value.Entry);
        }
        ...
        public override void Translate(IIntermediateInterfaceType @interface)
        {
            if (Options.AllowPartials || @interface.IsRoot)
            {
                ...
                if ((tc > 0 || mc > 0) && Options.AllowPartials || @interface.IsRoot)
                {
                    ...
                    if (Options.AllowPartials && (!@interface.IsRoot || @interface.Parts.Count > 0 || @interface.ForcedPartial))
                    {
                        this.WriteKeyword(CSharpKeywords.Partial);
                        this.Write(" ");
                    }
                    ...
                    if (@interface.ImplementedInterfaces.Count > 0 && (Options.AllowPartials && @interface.IsRoot || !Options.AllowPartials))
                    {
                        this.WriteOperator(" :");
                        this.DenoteNewLine();
                        IncreaseIndent();
                    ...
                    if (verbatim)
                    {
                        Translate((IIntermediateTypeParent)@interface);
                        foreach (var member in Options.AllowPartials ? @interface.Members.ExclusivelyOnParent(@interface) : from q in @interface.Members
                                                                                                                            select new KeyValuePair<IGeneralMemberUniqueIdentifier, MasterDictionaryEntry<IIntermediateMember>>(q.Key, new MasterDictionaryEntry<IIntermediateMember>(q.Value.Subordinate, (IIntermediateMember)q.Value.Entry)))
                            member.Value.Entry.Accept(this);
                    }
                    else if (alpha && specific)
                    {
                        Dictionary<DeclarationTranslationOrder, int> translationOrder = GetDistinctTranslationOrder();
                        foreach (var declaration in from declaration in
                                                        GetAllInterfaceDeclarations(@interface).ToArray()
                                                    orderby translationOrder[GetEntityOrder(declaration)],
                                                            declaration.Name
                                                    select declaration)
                            if (declaration is IIntermediateMember)
        ...
        public override void Translate(IIntermediateStructType @struct)
        {
            if (Options.AllowPartials || @struct.IsRoot)
            {
                ...
                if ((tc > 0 || mc > 0) && Options.AllowPartials || @struct.IsRoot)
                {
                    ...
                    if (Options.AllowPartials && (!@struct.IsRoot || @struct.Parts.Count > 0 || @struct.ForcedPartial))
                    {
                        this.WriteKeyword(CSharpKeywords.Partial);
                        this.Write(" ");
                    }
                    ...
                    if ((structAsIStruct.ImplementedInterfaces.Count > 0) && (Options.AllowPartials && @struct.IsRoot || !Options.AllowPartials))
                    {
                        this.WriteOperator(" :");
                        this.DenoteNewLine();
                        IncreaseIndent();
        ...
        private bool TranslateConstraints<TGenericParameter, TIntermediateGenericParameter, TParent, TIntermediateParent>(IIntermediateGenericParameter<TGenericParameter, TIntermediateGenericParameter, TParent, TIntermediateParent> parameter, bool last = false)
        ...
        public override void Translate<TField, TIntermediateField, TFieldParent, TIntermediateFieldParent>(IIntermediateFieldMember<TField, TIntermediateField, TFieldParent, TIntermediateFieldParent> field)
        {
            ...
            this.Translate(field.Metadata, MetadatumTargets.Field, false);
            ...
            if (field.Constant && !(field is IEnumFieldMember))
            {
                this.WriteKeyword(CSharpKeywords.Const);
                this.Write(" ");
            }
        ...
        public override void Translate(IIntermediateEnumFieldMember field)
        {
            ...
            {
                this.WriteOperator(" = ");
                ((IExpression)(field.Value)).Accept(this);
            }
            this.EndDeclarationDefinition();
        }
        ...
        public override void Translate<TIndexer, TIntermediateIndexer, TIndexerParent, TIntermediateIndexerParent>(IIntermediateIndexerMember<TIndexer, TIntermediateIndexer, TIndexerParent, TIntermediateIndexerParent> indexer)
        {
            ...
            if (indexer is IIntermediateClassIndexerMember)
                privateImplementation = (IInterfaceType)((IIntermediateClassIndexerMember)(indexer)).Implementations.FirstOrDefault();
            else if (indexer is IIntermediateStructIndexerMember)
                privateImplementation = (IInterfaceType)((IIntermediateStructIndexerMember)(indexer)).Implementations.FirstOrDefault();
            ...
            if (indexer.CanRead)
            {
                if (indexer.GetMethod.AccessLevel != AccessLevelModifiers.NoneOrUndefined && indexer.GetMethod.AccessLevel.CompareToEx(indexer.AccessLevel) < 0)
                    this.Translate(indexer.GetMethod.AccessLevel, indexer.GetMethod);
            ...
            if (indexer.CanWrite)
            {
                if (indexer.SetMethod.AccessLevel != AccessLevelModifiers.NoneOrUndefined && indexer.SetMethod.AccessLevel.CompareToEx(indexer.AccessLevel) < 0)
                    this.Translate(indexer.SetMethod.AccessLevel, indexer.SetMethod);
        ...
        public override void Translate<TMethod, TIntermediateMethod, TMethodParent, TIntermediateMethodParent>(IIntermediateMethodMember<TMethod, TIntermediateMethod, TMethodParent, TIntermediateMethodParent> method)
        {
            ...
            this.TranslateInternal(method, () =>
            {
                ...
                if (method is IIntermediateClassMethodMember)
                    flags = ((IIntermediateClassMethodMember)method).ImplicitAttributes;
                else if (method is IClassMethodMember)
                    flags = ((IClassMethodMember)method).Attributes;
                else if (method is IExtendedMethodMember)
                    flags = (ClassMethodMemberFlags)((IExtendedMethodMember)method).Attributes;
                else if (method is IInstanceMember)
                    flags = (ClassMethodMemberFlags)((IInstanceMember)method).Attributes;
                if ((flags & ClassMethodMemberFlags.Partial)            != ClassMethodMemberFlags.Partial &&
                    (flags & ClassMethodMemberFlags.PartialDefinition)  != ClassMethodMemberFlags.PartialDefinition)
                    /* In C#, partial methods cannot have access modifiers. */
                ...
                IIntermediateSignatureParent<IGeneralGenericSignatureMemberUniqueIdentifier, TSignature, TIntermediateSignature, TSignatureParameter, TIntermediateSignatureParameter, TParent, TIntermediateParent>
        {
            ...
            if (methodSignature.IsGenericConstruct)
            {
                ...
                bool isClassMember = methodSignature is IClassMethodMember;
                IExtendedInstanceMember ieim = isClassMember ? (IExtendedInstanceMember)methodSignature : null;
                bool isClassMethodMemberAbstract = ieim != null ? ieim.IsAbstract : false;
                foreach (var parameter in methodSignature.TypeParameters.Values)
                {
                    bool currentConstraints = this.TranslateConstraints(parameter, lastParam == parameter);
                    if (currentConstraints && !anyConstraints)
                 ...
                 * type-parameter. */
                //if (anyConstraints && methodSignature is IInstanceMember)
        ...
        {
            IInterfaceType privateImplementation = null;
            if (property is IIntermediateClassPropertyMember)
                privateImplementation = (IInterfaceType)((IIntermediateClassPropertyMember)(property)).Implementations.FirstOrDefault();
            else if (property is IIntermediateStructPropertyMember)
                privateImplementation = (IInterfaceType)((IIntermediateStructPropertyMember)(property)).Implementations.FirstOrDefault();
            ...
            if (property is IIntermediateInstanceMember)
            {
                ...
                else if (property is IClassPropertyMember)
                {
                    var cProp = property as IClassPropertyMember;
            ...
            if (property.CanRead)
            {
                if (property.GetMethod.AccessLevel != AccessLevelModifiers.NoneOrUndefined && property.GetMethod.AccessLevel.CompareToEx(property.AccessLevel) < 0)
            ...
            {
                if (property.SetMethod.AccessLevel != AccessLevelModifiers.NoneOrUndefined && property.SetMethod.AccessLevel.CompareToEx(property.AccessLevel) < 0)
            ...
            switch (parameter.Direction)
            {
                ...
                case ParameterCoercionDirection.In:
                    if (parameter.ParameterType            != null  &&
                        ...
                        parameter.Parent.Parameters        != null  &&
                        parameter.Parent.Parameters.Values != null  &&
                        parameter.Parent.Parameters.Values[parameter.Parent.Parameters.Count - 1] == parameter)
                    {
                        var idm = parameter.ParameterType.IdentityManager;
                        if (idm is ICliManager)
                        {
                            var icm = (ICliManager)idm;
                            if (parameter.Metadata.Contains(icm.ObtainTypeReference(icm.RuntimeEnvironment.ParamArrayMetadatum)))
                            {
                                this.WriteKeyword(CSharpKeywords.Params);
                                this.Write(" ");
                            }
                        }
                    }
                    ...
                    if (parameter.Parent is IIntermediateClassMethodMember)
                    {
                        ...
                        if (mParent.IsExtensionMethod)
                        {
                            if ((mParent.Parent.SpecialModifier & SpecialClassModifier.Static) == SpecialClassModifier.Static)
                                ...
                                this.Translate(new CommentExpression("Extension methods must be in static classes.", DecorationDisplaySide.Before));
                            this.Write(" ");
                        }
                    }
                    first = false;
        ...
        public override void Translate(ILockStatement statement)
        {
            ...
            this.Write(" ");
            this.WriteOperator("(");
            statement.MonitorLock.Accept(this);
            this.WriteOperator(")");
            this.DenoteNewLine();
            this.TranslateBlockInternal(statement, true);
        }

        public override void Translate(IDefaultValueExpression expression)
        {
            if (expression == null)
                throw new ArgumentNullException("expression");
            this.WriteKeyword(CSharpKeywords.Default);
            this.WriteOperator("(");
            this.Translate(expression.TypeToDefault);
            this.WriteOperator(")");
        }
        ...
        public override void Translate(IAwaitStatementExpression expression)
        {
            this.Translate((IAwaitExpression)expression);
        }
        public override void Translate(IAwaitExpression expression)
        {
            this.WriteKeyword(CSharpKeywords.Await);
            this.Write(" ");
            expression.Proffer.Accept(this);
        }

        public override void Translate(IExplicitStringLiteralDecorationExpression expression)
        {
            if (expression == null)
                throw new ArgumentNullException("expression");
            this.Write(expression.LiteralInsert);
        }

        public override void Translate(IBlockExpression expression)
        {
            if (expression != null && expression.Block != null)
                expression.Block.Accept(this);
        }

        public override void Translate(IExplicitStringLiteralStatement statement)
        {
            this.Write(statement.Literal);
            this.WriteLine();
        }
