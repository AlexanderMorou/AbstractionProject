...
{
    internal class CliMetadataModuleTableReader :
        CliMetadataLazyTable<ICliMetadataModuleTableRow>, 
        ICliMetadataTable, 
        ICliMetadataModuleTable
    {
        ...
        /// <summary>
        /// Size constant used when the total size of the <see cref="CliMetadataModuleLockedTableRow"/>
        /// is 10 bytes long.
        /// </summary>
        internal const int __COR_MODULE_CALC_SIZE_1__ = 10;
        /// <summary>
        /// Size constant used when the total size of the <see cref="CliMetadataModuleLockedTableRow"/>
        /// is 16 bytes long.
        /// </summary>
        internal const int __COR_MODULE_CALC_SIZE_2__ = 16;
        /// <summary>
        /// Size constant used when the total size of the <see cref="CliMetadataModuleLockedTableRow"/>
        /// is 12 bytes long.
        /// </summary>
        internal const int __COR_MODULE_CALC_SIZE_3__ = 12;
        /// <summary>
        /// Size constant used when the total size of the <see cref="CliMetadataModuleLockedTableRow"/>
        /// is 18 bytes long.
        /// </summary>
        internal const int __COR_MODULE_CALC_SIZE_4__ = 18;
        private int __size;
        private byte state;
        /// <summary>
        /// Data member which denotes where in the original stream the <see cref="CliMetadataModuleTableReader"/>
        /// is.
        /// </summary>
        ...
        public override CliMetadataTableKinds Kind
        {
            get
            {
                return CliMetadataTableKinds.Module;
            }
        }
        public long Length
        {
        ...
        protected override ICliMetadataModuleTableRow ReadElementAt(uint index)
        {
            bool gotLock = false;
            ...
            ushort generation = this.reader.ReadUInt16();
            uint nameIndex;
            switch (this.state)
            {
                case 2: case 3:
            ...
            uint moduleVersionIndex;
            uint encIdIndex;
            uint encBaseIdIndex;
            switch (this.state)
            {
                case 1: case 3:
                    moduleVersionIndex = this.reader.ReadUInt32();
                    encIdIndex = this.reader.ReadUInt32();
                    encBaseIdIndex = this.reader.ReadUInt32();
                    break;
                default:
                    moduleVersionIndex = this.reader.ReadUInt16();
                    encIdIndex = this.reader.ReadUInt16();
                    encBaseIdIndex = this.reader.ReadUInt16();
                    break;
            }
            if (gotLock)
                System.Threading.Monitor.Exit(this.syncObject);
            return new CliMetadataModuleLockedTableRow(index, this.state, this.metadataRoot, generation, nameIndex, moduleVersionIndex, encIdIndex, encBaseIdIndex);
        }
        public override void Read()
        {
            ...
            for (uint index = 1; index <= this.Count; index++)
            {
                ...
                if (!base.ItemLoaded(index))
                {
                    ...
                    base.InjectLoadedItem(new CliMetadataModuleLockedTableRow(index, this.state, this.metadataRoot, generation, nameIndex, moduleVersionIndex, encIdIndex, encBaseIdIndex), index);
                }
                if (gotLock)
                    System.Threading.Monitor.Exit(this.syncObject);
            }
        ...
        public CliMetadataModuleTableReader(ICliMetadataRoot metadataRoot, Tuple<object, FileStream, EndianAwareBinaryReader> readerInfo, uint rowCount)
