...
namespace AllenCopeland.Abstraction.OldCodeGen.Translation
{
        ...
        IntermediateCodeTranslator
    {
        ...
        public override void TranslateType(IClassType classType)
        {
            ...
            {
                /* *
                 * Translate the comments associated to the class.
                 * */
                TranslateAutoComments(classType, () => (GetGenericTypeParametersDocComments(classType)));
                /* *
        ...
        {
            /* *
             * Translate the comments associated to the enumerator.
             * */
            TranslateAutoComments(enumeratorType);
        ...
        {
            /* *
             * Translate the comments associated to the delegate.
             * */
            TranslateAutoComments(delegateType, () => (GetGenericTypeParametersDocComments<CodeTypeDelegate>(delegateType)));
        ...
        public override void TranslateType(IInterfaceType interfaceType)
        {
            ...
            {
                /* *
                 * Translate the comments associated to the interface.
                 * */
                TranslateAutoComments(interfaceType, () => (GetGenericTypeParametersDocComments(interfaceType)));
        ...
        public override void TranslateType(IStructType structureType)
        {
            ...
            {
                /* *
                 * Translate the comments associated to the struct.
                 * */
                TranslateAutoComments(structureType, () => (GetGenericTypeParametersDocComments(structureType)));
                //The attributes defined, but only on the root version, because
                //if certain attributes are redeclared, you get runtime errors 
                //(see: AttributeUsageAttribute.AllowMultiple).
        ...
        protected override void TranslateProjectInner(IIntermediateProject project)
        {
            ...
            if (base.Options.AutoResolveReferences)
            {
                ...
                if (!base.Options.SuppressTailGenerationMessage)
        ...
        private string GetGenericTypeParametersDocComments<TDom>(IParameteredDeclaredType<TDom> type)
            where TDom :
                CodeTypeDeclaration
        {
            if (Options.AutoComments)
            {
                ...
                bool first = true;
                foreach (var tParam in type.TypeParameters.Values)
                {
                    if (!string.IsNullOrEmpty(tParam.DocumentationComment))
                    {
                        if (first)
                            first = false;
                        else
        ...
        private string GetParameterDocumentationComments<TParameter, TSignatureDom, TParent>(IParameteredDeclaration<TParameter, TSignatureDom, TParent> target)
            where TParameter :
                ...
                CodeObject
        {
            if (Options.AutoComments)
            {
                StringBuilder result = new StringBuilder();
                bool first = true;
                foreach (var param in target.Parameters.Values)
                ...
                IDeclarationTarget
        {
            ...
            {
                StringBuilder result = new StringBuilder();
                result.Append(GetParameterDocumentationComments(ambigMethodSigMember));
                bool first = true;
                foreach (var tParam in ambigMethodSigMember.TypeParameters.Values)
                    if (!string.IsNullOrEmpty(tParam.DocumentationComment))
                    {
                        if (first)
                            first = false;
        ...
        {
            TranslateAutoComments(indexerSigMember, () => GetParameterDocumentationComments(indexerSigMember));
        ...
        {
            TranslateAutoComments(propertyMember, () => GetParameterDocumentationComments(propertyMember));
        ...
        public override void TranslateMember<TParent>(IPropertySignatureMember<TParent> ambigPropertySigMember)
        {
            ...
            else if (ambigPropertySigMember.HidesPrevious)
            {
                this.TranslateConceptKeyword(Keywords.New);
                base.Write(" ", TranslatorFormatterTokenType.Other);
            }
        ...
        private void TranslateAutoComments(IAutoCommentType type, Func<string> innerComment = null)
        {
            if (Options.AutoComments)
            {
        ...
        {
            this.TranslateAutoComments(constructorMember, ()=> GetParameterDocumentationComments(constructorMember));
        ...
        public override void TranslateStatement(ISwitchStatement switchStatement)
        {
            ...
            foreach (ISwitchStatementCase @case in switchStatement.Cases)
            {
                ...
                if (!ScanForLogicalBreaks(@case.Statements))
                {
                    //CS0163 - Control cannot fall through from one case label ('case.Cases') to another
                    TranslateConceptComment("Generated object graph missing break statement.", false);
                    TranslateConceptKeyword(Keywords.Break);
                    base.WriteLine(";", TranslatorFormatterTokenType.Operator);
                }
                base.DecreaseIndent();
        ...
        private bool ScanForLogicalBreaks(IStatementBlock block)
        {
            foreach (var statement in block)
                if (ScanForLogicalBreaks(statement))
                    return true;
            return false;
        }

        private bool ScanForLogicalBreaks(IStatement statement)
        {
            if (statement is IConditionStatement)
            {
                var conditionStatement = (IConditionStatement) statement;
                return ScanForLogicalBreaks(conditionStatement.Statements) &&
                       ScanForLogicalBreaks(conditionStatement.FalseBlock);
            }
            else if (statement is ISwitchStatement)
                return ((ISwitchStatement) (statement)).Cases.All(p => ScanForLogicalBreaks(p.Statements));
            else if (statement is IBreakStatement || statement is IReturnStatement ||
                     statement is IYieldBreakStatement ||
                     statement is IGoToLabelStatement ||
                     statement is IThrowStatement)
                return true;
            return false;
        }
        ...
        public override void TranslateStatementBlock(IStatementBlock statementBlock)
        {
            if (statementBlock.Parent is IBlockStatement || 
               ...
               (!(
                    statementBlock.Parent is IConditionStatement ||
                    statementBlock.Parent is IEnumeratorStatement)) ||
                statementBlock.Locals.Count > 0)
            {
                base.WriteLine("{", TranslatorFormatterTokenType.Operator);
            }
            if (!(statementBlock.Parent is IBlockStatement))
            ...
            if (!(statementBlock.Parent is IBlockStatement))
                base.DecreaseIndent();
            if (statementBlock.Parent is IBlockStatement || (statementBlock.Parent is IConditionStatement) && (statementBlock.Count != 1) || 
                (!(
                    statementBlock.Parent is IConditionStatement ||
                    statementBlock.Parent is IEnumeratorStatement)) ||
                statementBlock.Locals.Count > 0)
        ...
        public override void TranslateExpression(IShiftExpression expression)
        {
            base.Write("(", TranslatorFormatterTokenType.Operator);
            this.TranslateExpression(expression.LeftSide);
            ...
            this.TranslateExpression(expression.RightSide);
            base.Write(")", TranslatorFormatterTokenType.Operator);
        }
        ...
        public override void TranslateExpression(IMethodReferenceExpression methodRefExpression)
        {
            ...
            if (methodRefExpression is MethodMember.ReferenceExpression)
            {
                if (includeReference)
                {
                    ...
                    MethodMember mm = mmre.referencePoint as MethodMember;
                    if (mm != null && (mm.PrivateImplementationTarget == null) || (mmre.Reference is ICastExpression))
                    {
                    ...
                    {
                        this.TranslateExpression(new CastExpression(methodRefExpression.Reference, mm.PrivateImplementationTarget));
                        base.Write(".", TranslatorFormatterTokenType.Operator);
                    }
                }
        ...
        public override void TranslateExpression(IParameterReferenceExpression paramRefExpression)
        {
            ...
            else if (paramRefExpression is ConstructorParameterMember.ReferenceExpression)
                this.TranslateConceptIdentifier(((ConstructorParameterMember.ReferenceExpression) paramRefExpression).referencePoint, false);
            else
        ...
        public override string SubToolName
        {
            get
            {
                return "Abstraction's Old C\u266F Code Translator";
            }
        }
        ...
        public override void TranslateExpression(IDefaultValueExpression expression)
        {
            TranslateConceptKeyword(Keywords.Default);
            base.Write("(", TranslatorFormatterTokenType.Operator);
            TranslateConceptTypeName(expression.DefaultType);
            base.Write(")", TranslatorFormatterTokenType.Operator);
        }

        public override void TranslateStatement(IThrowStatement throwStatement)
        {
            TranslateConceptKeyword(Keywords.Throw);
            base.Write(" ", TranslatorFormatterTokenType.Other);
            TranslateExpression(throwStatement.ExceptionExpression);
            if (!suppressLineTerminator)
                base.WriteLine(";", TranslatorFormatterTokenType.Operator);
        }
        ...
        {
            TranslateConceptKeyword(Keywords.Lock);
            base.Write("(", TranslatorFormatterTokenType.Operator);
            TranslateExpression(lockStatement.LockedExpression);
            base.WriteLine(")", TranslatorFormatterTokenType.Operator);
            TranslateStatementBlock(lockStatement.Statements);
        }
