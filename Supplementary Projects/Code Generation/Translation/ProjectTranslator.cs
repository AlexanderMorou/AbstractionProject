...
namespace AllenCopeland.Abstraction.OldCodeGen.Translation
{
    ...
    {
        public static string GetFileNameFor(IDeclaredType type, string relativeRoot, IIntermediateProject project, string extension, IIntermediateCodeTranslatorOptions options, bool invertSeparator)
        {
            ...
            var uptrail = @"..\".Repeat(Path.GetDirectoryName(currentFile).Substring(actualRelativeRoot.Length).Split(new Char[] { Path.DirectorySeparatorChar }, StringSplitOptions.RemoveEmptyEntries).Length);
            if (invertSeparator)
