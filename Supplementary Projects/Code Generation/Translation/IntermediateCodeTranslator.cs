...
namespace AllenCopeland.Abstraction.OldCodeGen.Translation
{
        ...
        IIntermediateCodeTranslator
    {
        ...
        protected static string ResolveDocumentationCommentLookups(string target)
        {
            ...
            StringBuilder result = new StringBuilder();
            int lastStartIndex = 0;
            var tabledTarget = BreakdownTables(BreakdownWrap(target));
            foreach (var match in docCommentLookup.MatchSet(tabledTarget))
            {
                result.Append(tabledTarget.Substring(lastStartIndex, match.Index - lastStartIndex));
            ...
            result.Append(tabledTarget.Substring(lastStartIndex));
            return result.ToString();
        }
