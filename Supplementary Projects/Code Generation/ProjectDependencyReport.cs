...
namespace AllenCopeland.Abstraction.OldCodeGen
{
    public struct ProjectDependencyReport
    {
        ...
        private bool recurseReferences;
            ...
            foreach (ITypeReference itr in this.sourceData)
            {
                ...
                if (it is IExternType)
                {
                    ...
                    if ((!(result.Contains(((IExternType) (it)).Type.Assembly))))
                        InsertAssemblyWithReferences(result, ((IExternType) (it)).Type.Assembly, this.recurseReferences);
                }
                else
                    continue;
            }
        ...
        private static void InsertAssemblyWithReferences(List<Assembly> result, Assembly a, bool recurseReferences)
        {
            ...
            if (recurseReferences)
                foreach (AssemblyName reference in dependencies)
                {
                    ...
                    {
                        InsertAssemblyWithReferences(result, referencedAssembly, recurseReferences);
