        ...
        IStatementVisitor<bool, TContext>
    {
        ...
        public bool IsBrokeFull(IControlledCollection<IStatement> series, TContext context)
        {
            foreach (var blockStatement in series)
                if (blockStatement.Visit(this, context))
                    return true;
            return false;
        }
        ...
        {
            return IsBrokeFull(statement, context) && (statement.HasNext && statement.Next.Visit(this, context));
        }
        ...
        {
            return statement.All(c => c.Visit(hardVisitor, context)) && statement.DefaultBlock != null;
        }

        public bool Visit(ITryStatement statement, TContext context)
        {
            return IsBrokeFull(statement, context) && statement.Values.All(clause => clause.Visit(this, context)) && (statement.HasCatchAll && statement.CatchAll.Visit(this, context) || !statement.HasCatchAll) && (statement.HasFinally && statement.Finally.Visit(this, context) || !statement.HasFinally);
        }
